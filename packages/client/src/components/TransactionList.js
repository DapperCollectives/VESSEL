...
const Column = ({ className = "", children, style = {} }) => (
  <div 
    className={`column is-flex is-align-items-center ${className}`} 
    style={style}>
    {children}
  </div>
);
...
const TransactionListItem = ({
  ...
  return (
    <div className={classes}>
      ...
      <Column>
        {getUserFriendlyIntent(intent)}
      </Column>
      <Column 
        className="is-hidden-mobile" 
        style={{ maxWidth: maxWidths.status }}>
        <TransactionStatusIcon status={status} />
        <span className="ml-2 is-capitalized">{status}</span>
      </Column>
      <Column 
        className="is-hidden-mobile has-text-grey" 
        style={{ maxWidth: maxWidths.date }}>
...
const TransactionList = ({ safeData, transactions = [] }) => {
  ...
  return (
    <div className="border-light rounded-sm">
      <div className="is-flex has-text-grey border-light-bottom is-hidden-mobile">
        ...
        <Column>Info</Column>
        <Column style={{ maxWidth: maxWidths.status }}>
          Status
        </Column>
        <Column style={{ maxWidth: maxWidths.date }}>
          Date
        </Column>
