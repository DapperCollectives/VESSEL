...
const ReviewRemoveSafeOwner = ({
  ...
  const { proposeRemoveSigner, setTreasury } =
  ...
  const onSubmitClick = async () => {
    await proposeRemoveSigner(formatAddress(owner.address), newThreshold);
    setTreasury(address, {
      safeOwners: safeOwners.filter((o) => o.address !== owner.address),
    });
    closeModal();
  ...
  return (
    <>
      ...
      <div className="border-light-top p-4 has-text-grey">
        <p className="has-text-grey">Remove Owner</p>
        <div className="pb-4">
...
export default ReviewRemoveSafeOwner;
