...
// Proposed ACTION: Transfer `amount` FlowToken from the DAOTreasuryV4
...
transaction(treasuryAddr: Address, recipientAddr: Address, identifier: String, amount: UFix64, message: String, keyIds: [UInt64], signatures: [String], signatureBlock: UInt64) {
  ...
  prepare(signer: AuthAccount) {
    self.treasury = getAccount(treasuryAddr).getCapability(DAOTreasuryV4.TreasuryPublicPath)
                    ...
                    ?? panic("A DAOTreasuryV4 doesn't exist at the treasuryAddr")
    ...
    self.action = TreasuryActionsV4.TransferTokenToTreasury(recipientTreasury: self.recipientTreasury, identifier: identifier, amount: amount, proposer: signer.address)
